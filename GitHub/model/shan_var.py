...
# ==========================================
# 2. 核心模块: SE-mSEM (Attention-Enhanced mSEM)
# ==========================================

# [新增] SE Block - 用于对特征图(Feature Map)进行通道加权
...
class mSEM(nn.Module):
    def __init__(self, F1, D, num_channels, region_indices):
        ...
        # 2. [核心改进] SE Attention
        # 我们对 "Temporal Filters (F1)" 这个维度进行重校准
        # 作用：让网络判断哪些频率/时序模式在当前的拼接特征中是有效的
        self.se = SEBlock(F1, reduction=4)

        # 3. Global Spatial Convolution
        # 输入通道数 = 原始通道数 (C) + 脑区数 (k)
    ...
    def forward(self, x):
        ...
        # Step C: [核心改进] SE 加权
        # 在进入全局卷积之前，清洗特征，抑制噪声响应
        concat_x = self.se(concat_x)
...
class mSEM_TransNet(nn.Module):
    def __init__(self, num_classes=4, num_samples=1000, num_channels=22, embed_dim=32, pool_size=50,
        ...
        # 3. mSEM 模块 (替换 Spatial Conv)
        # 这里的 mSEM 已经包含了 SE Attention
    ...
    def forward(self, x):
        ...
        # 2. mSEM Spatial Extraction (with SE)
        x = self.msem(x)
