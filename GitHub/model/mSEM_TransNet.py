...
class EntropyPool1d(nn.Module):
    """香农熵池化：提取非线性复杂度特征，与VarPool形成特征正交"""
    ...
    def forward(self, x):
        ...
        for i in range(out_shape):
            index = i * self.stride
            input_window = x[:, :, index:index + self.kernel_size]  # (B, C, kernel_size)

            # 1. 计算局部能量
            energy = input_window ** 2

            # 2. 转化为概率分布 (在时间窗口维度上归一化)
            prob = energy / (energy.sum(dim=-1, keepdim=True) + eps)

            # 3. 计算香农熵: -sum(p * log(p))
            entropy = -torch.sum(prob * torch.log(prob + eps), dim=-1, keepdim=True)
            out.append(entropy)

        out = torch.cat(out, dim=-1)  # (B, C, out_shape)
        return out
...
# ==========================================
# 2. 核心模块: mSEM (Multi-level Spatial Embedding Module)
# ==========================================
...
class mSEM(nn.Module):
    def __init__(self, F1, D, num_channels, region_indices):
        ...
        region_indices: 脑区分组索引
        """
        super(mSEM, self).__init__()
        self.region_indices = region_indices
        self.F1 = F1

        # 1. Local Spatial Convolutions (针对每个脑区，独立提取局部拓扑)
    ...
    def forward(self, x):
        ...
        # Step A: 提取局部脑区特征 (Path A: Local Micro)
        for i, idx in enumerate(self.region_indices):
            region_x = x[:, :, idx, :]
            feat = self.local_convs[i](region_x)  # (B, F1, 1, T)
            local_features.append(feat)

        # Step B: 拼接 (Path A + Path B: Global Macro)
        local_features_tensor = torch.cat(local_features, dim=2)  # (B, F1, 3, T)
        concat_x = torch.cat([x, local_features_tensor], dim=2)  # (B, F1, 25, T)
...
class mSEM_TransNet(nn.Module):
    def __init__(self, num_classes=4, num_samples=1000, num_channels=22, embed_dim=32, pool_size=50,
        ...
        # 4. 双流特征解耦后端 (Var 流 + Entropy 流)
        self.entropy_pool = EntropyPool1d(pool_size, pool_stride)
    ...
    def forward(self, x):
        ...
        # 3. Dual Pooling (熵流计算与方差流计算并行)
        x1 = self.entropy_pool(x)
        x2 = self.var_pool(x)
