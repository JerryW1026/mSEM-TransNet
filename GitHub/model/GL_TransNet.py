...
# ==========================================
# 1. 基础组件 (SE, Attention, Pooling, Transformer)
# ==========================================

# [核心组件] Squeeze-and-Excitation Block
# 用于动态调整 Global 和 Local 分支的特征权重
...
# ==========================================
# 2. 通用特征提取器 (Feature Extractor)
# ==========================================
class FeatureExtractor(nn.Module):
    def __init__(self, num_channels, embed_dim):
        super().__init__()
        # 为了控制参数量，每个分支内部的 channel 数设为 embed_dim // 4
        f = embed_dim // 4
...
# ==========================================
# 3. 终极模型：GL_TransNet (with SE Attention)
# ==========================================
class GL_TransNet(nn.Module):
    def __init__(self, num_classes=4, num_samples=1000, num_channels=22, embed_dim=32, pool_size=50,
        ...
        # 2. 四路提取器 (全独立)
        # Global: 处理所有 22 通道 (Baseline 能力保底)
        ...
        # [核心组件] 引入 SE Block 进行注意力加权融合
        # reduction=8 用于平衡参数量和效果
        self.se_block = SEBlock(self.fusion_dim, reduction=8)
    ...
    def forward(self, x):
        ...
        # 3. 强力融合
        # 拼接 -> (B, 128, 1, T)
        x = torch.cat([feat_G, feat_L, feat_M, feat_R], dim=1)

        # [SE Attention]
        # 网络会在这里自动学习各分支的权重
        x = self.se_block(x)

        x = x.squeeze(dim=2)  # (B, 128, T)
