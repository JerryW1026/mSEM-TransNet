...
# ==========================================
# 2. mSEM_TransNet (SEED-IV 62通道 左/中/右 三分区定制版)
# ==========================================
class mSEM_TransNet_SEEDIV62(nn.Module):
    def __init__(self, num_classes=4, num_samples=200, num_channels=62, embed_dim=32, pool_size=50,
                 pool_stride=15, num_heads=8, fc_ratio=4, depth=4, attn_drop=0.5, fc_drop=0.5):
        super().__init__()

        # SEED 标准 62 通道拓扑结构: 左半脑, 中轴, 右半脑
        self.idx_left = [0, 3, 5, 6, 7, 8, 14, 15, 16, 17, 23, 24, 25, 26, 32, 33, 34, 35, 41, 42, 43, 44, 50, 51, 52,
                         57, 58]
        self.idx_mid = [1, 9, 18, 27, 36, 45, 53, 59]
        self.idx_right = [2, 4, 10, 11, 12, 13, 19, 20, 21, 22, 28, 29, 30, 31, 37, 38, 39, 40, 46, 47, 48, 49, 54, 55,
                          56, 60, 61]
...
# 3. 数据加载模块 (读取 62 全通道)
# ==========================================
class SEEDIVDataset62Ch(Dataset):
    def __init__(self, file_info_list, window_size=200, stride=200):
        self.window_size = window_size
        self.stride = stride

        self.session_labels = {
            0: [1, 2, 3, 0, 2, 0, 0, 1, 0, 1, 2, 1, 1, 1, 2, 3, 2, 2, 3, 3, 0, 3, 0, 3],
            1: [2, 1, 3, 0, 0, 2, 0, 2, 3, 3, 2, 3, 2, 0, 1, 1, 2, 1, 0, 3, 0, 1, 3, 1],
            2: [1, 2, 2, 1, 3, 3, 3, 1, 1, 2, 1, 0, 2, 3, 3, 0, 2, 3, 0, 0, 2, 0, 1, 0]
        }

        self.data, self.labels = [], []

        for filepath, session_idx in file_info_list:
            if os.path.exists(filepath):
                d, l = self._load_and_epoch(filepath, session_idx)
    ...
    def _load_and_epoch(self, filepath, session_idx):
        mat_data = sio.loadmat(filepath)
        keys = list(mat_data.keys())

        # 利用正则表达式匹配并精确排序 eeg 字典键值
        ...
        for trial_idx, key in enumerate(trial_keys):
            ...
            current_label = current_session_labels[trial_idx]
...
# ==========================================
# 4. 跨会话评估逻辑 (2 Train / 1 Test)
# ==========================================
def train_and_evaluate_seed4(train_info_list, test_info_list, device, epochs=50, batch_size=64, lr=5e-4):
    train_dataset = SEEDIVDataset62Ch(train_info_list)
    test_dataset = SEEDIVDataset62Ch(test_info_list)

    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)

    # 启用 62通道定制网络，保持强大的抗过拟合配置
    model = mSEM_TransNet_SEEDIV62(num_classes=4, attn_drop=0.6, fc_drop=0.7).to(device)
...
if __name__ == '__main__':
    # 📌 请确认此路径指向您的 eeg_raw_data 目录
    data_dir = r"C:\Users\WJR\Desktop\分区+多特征融合实验\EEG-TransNet-main\EEG-TransNet-main -+分区\SEED_IV\SEED_IV\eeg_raw_data"
    ...
    for subject_id in range(1, 16):
        print(f"\n{'-' * 70}")
        print(f" 🚀 开始评估被试 (Subject) {subject_id}/15 [SEED-IV | 4分类 | 62通道三分区 | Raw EEG]")

        session1_pattern = os.path.join(data_dir, '1', f"{subject_id}_*.mat")
        session2_pattern = os.path.join(data_dir, '2', f"{subject_id}_*.mat")
        session3_pattern = os.path.join(data_dir, '3', f"{subject_id}_*.mat")

        file1 = glob.glob(session1_pattern)
        file2 = glob.glob(session2_pattern)
        file3 = glob.glob(session3_pattern)

        if not (file1 and file2 and file3):
            print(f"[警告] 被试 {subject_id} 的数据缺失，已跳过。请检查 1, 2, 3 文件夹。")
            continue

        train_info_list = [(file1[0], 0), (file2[0], 1)]
        test_info_list = [(file3[0], 2)]

        # 建议保持 100 轮：62通道的数据维度激增了 5 倍，模型需要更长的 Epoch 寻找全局最优
        acc = train_and_evaluate_seed4(train_info_list, test_info_list, device, epochs=100)

        all_subject_acc.append(acc)
        print(f" 🏆 被试 {subject_id} 跨会话最佳准确率: {acc:.2f}%")
    ...
    if len(all_subject_acc) > 0:
        ...
        print(f"\n{'=' * 70}")
        print(" 🎉 SEED-IV (4分类) 15 名被试跨会话实验全部完成！")
        print(f" 🎯 采用配置: 62 全通道左中右三分区 (无 DSP外挂的纯 Raw EEG 端到端解码)")
